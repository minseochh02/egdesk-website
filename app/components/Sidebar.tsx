  ...
  return (
    <div className={`flex h-full flex-col border-r border-zinc-800 bg-zinc-950 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
      ...
      <div className={`border-t border-zinc-800 ${isCollapsed ? 'p-2' : 'p-4'}`}>
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} ${isCollapsed ? '' : 'mb-3'}`}>
        ...
        {!isCollapsed && (
          <button
            onClick={onSignOut}
            className="w-full px-3 py-2 text-sm text-red-400 hover:bg-zinc-800 rounded-lg transition-colors flex items-center gap-2"
          >
